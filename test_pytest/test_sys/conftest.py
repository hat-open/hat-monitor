...
def monitor_factory(tmp_path, unused_tcp_port_factory):
    ...
    def run_monitor(parent_infos=[], default_algorithm='BLESS_ALL',
        ...
        process = test_sys.common.run_monitor_subprocess(
            conf=test_sys.common.create_monitor_conf(
        ...
        while not test_sys.common.process_is_running(process):
            time.sleep(0.1)
        while not process.connections():
            time.sleep(0.1)

        return test_sys.common.ServerInfo(
            process=process,
            monitor_port=monitor_port,
            master_port=master_port,
            ui_port=ui_port,
    ...
    for process in processes:
        test_sys.common.stop_process(process)
...
async def component_factory():
    ...
    async def create_component(name, group, server_info,
        ...
        component = await test_sys.common.create_component_client(
...
async def ui_client_factory():
    ...
    async def create_ui_client(server_info):
        ...
        client = await test_sys.common.create_ui_client(address)
...
def cluster_factory(monitor_factory, component_factory, ui_client_factory):
    async def create_cluster(group_conf,
        ...
        return test_sys.common.Cluster(
